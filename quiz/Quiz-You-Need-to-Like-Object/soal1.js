...
function ganjilGenapAsianGames(date, data) {
    ...
    let result = 0
    let odd = (date % 2 !== 0)
    if (odd) { // Search for even plat's number
        for (i = 0; i < data.length; i++){
            dataPlat = data[i].plat;
            dataPlat = dataPlat.split(" ")
            dataPlat = Number(dataPlat[1]);
            if (dataPlat % 2 === 0 && data[i].type === 'Mobil') result++;
        }
    }
    else if (!odd) { // Search for odd plat's number
        for (i = 0; i < data.length; i++){
            dataPlat = data[i].plat;
            dataPlat = dataPlat.split(" ")
            dataPlat = Number(dataPlat[1]);
