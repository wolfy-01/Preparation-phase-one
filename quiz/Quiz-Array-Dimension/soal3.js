...
function minDistanceBetweenGreatest(arr) {
    let count = 0;
    let distance = [];
    greatNum = Math.max(...arr);
    for (i = arr.indexOf(greatNum); i <= arr.length-1; i++){
        if (arr[i] !== greatNum) count++;
        else if (arr[i] === greatNum && i !== arr.indexOf(greatNum)){
            distance.push(count);
            count = 0;
        }
    }
    if (distance.length === 0) return 0;
    let result = (Math.min(...distance) + 1);
    return result;
  }
