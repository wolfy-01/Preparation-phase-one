...
function missingNum(arr) {
    ...
    for (i = 0; i <= arr.length-1; i++){
        for (j = 0; j <= arr[i].length-1; j++){
            if (typeof(arr[i][j]) === `number` ) numArr.push(arr[i][j]); // Filtering the array to become one-dimension array and contains number only
        }
    }
    numArr = numArr.sort((a,b) => a-b);
    for (i = 0; i <= numArr.length-2; i++){
        if (numArr[i+1] !== numArr[i]+1){ // Check if the next element is equal to current element + 1
             result.push(numArr[i]+1);
             numArr.splice(i+1,0, numArr[i]+1) // Filling the empty number 
            }
    }
    return result;
  }
